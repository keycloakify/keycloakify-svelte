...
function parse(versionStr) {
  const match = versionStr.trim().match(/^([0-9]+)\.([0-9]+)\.([0-9]+)(?:-rc\.([0-9]+))?$/);
...
function compare(v1, v2) {
  ...
  const noUndefined = (/** @type {number | undefined} */ n) => (n === undefined ? Infinity : n);
...
const is_pre_release = is_upgraded_version === 'false' ? 'false' : toVersion.rc !== undefined ? 'true' : 'false';
